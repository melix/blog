...
But knowing that the JDK has an embedded HTTP server, and that there's a lot of hype around Graal those days, I wanted to see if we could achieve the same thing, with a fast startup, with Graal.
...
In the end it would be nice if Graal could completely eliminate the need for having this class, but until then I just cannot recommend anyone to use Groovy to build native images: it's just _too frustrating_.
