...
Those are not specific to Testcontainers, but they led us to using it in a _different fashion_ internally:

- tests need to deal with the container lifecycle, which involves things like static initializers, test setup and cleanup, etc. : while this is completely acceptable, you can see the Testcontainers API as a "low-level" API. We don't want users to deal with the lifecycle of containers, or have to think about how to properly setup their tests. There are annotations like `@TestContainers` which are helping, but conflict with other annotations like `@MicronautTest` which make things harder than expected. Long story short, we'd like to have test containers management _transparent_.
- Testcontainers APIs "leak" to the test classpath, making it impossible to https://graalvm.github.io/native-build-tools/latest/gradle-plugin.html#testing-support[run tests in native mode]. Again this is not specific to the Testcontainers library: many libraries are not yet compatible with GraalVM. Making your tests work with Testcontainers in native can be really tricky, especially because of transitive dependencies or JDBC drivers, which are often not compatible. The effort can be massive "just" to get tests running natively.
- You need to figure out the specific test containers dependencies you need for your application and those dependencies may have transitive dependencies which conflict with the ones you use (both in production or tests). In general, this isn't a big deal, but it adds a bit of cognitive overhead and noise in build scripts.
- It's very difficult, or sometimes even impossible, to share containers between distinct projects (for example, an MQTT client and a server may want to use the same container)
- It simply doesn't work for "development mode", when you run the application locally (not tests, but really running the application): in this case, you need to inject the Testcontainers runtime in your application classpath, which can lead to dependency conflicts or resource leaks.
