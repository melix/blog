...
Groovy is primarily a dynamic language. It's the most widely used alternative language on the JVM, with use cases ranging
...
Those two classes define the same `getName` method, but it is not defined explicitly in a contract (for example through an interface). Imagine that you have a list of objects containing either ducks, cats, or anything else definining a `getName` method. Then a dynamic language will let you call
that method:
